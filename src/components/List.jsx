...
const List = () => {
  ...
  function handleCkeck(e){
    ...
    console.log(list[index].isCompleted)
    let newtodos = [...list]
  ...
  return (
    <div className="bg-blue-200 min-h-[80vh] w-[80vw] m-auto mt-4 rounded-xl px-6 py-4 flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        {list.map((item) => {
          return (<div key={item.id} className="flex justify-between bg-blue-500 px-6 py-3 items-center rounded-md text-white font-bold text-lg">
            <div className="flex gap-2 items-center">
              <input type="checkbox" name={item.id} value={item.isCompleted} onChange={handleCkeck}/>
              <h2 className={item.isCompleted?"line-through text-black":""}>{item.todo}</h2>
            </div>
